...
public class IARIAuthProcessor implements ProcessingResult {
    ...
    public int process(InputStream is) {
        ...
        SignatureValidator signatureValidator = new SignatureValidator(trustStore,
                authDocument.getDocument(), authDocument.getSignatureNode(), expectedRefs,
                authDocument.authType, authDocument.range);
    ...
    private int checkSignatureProperties(int sigStatus) {
        ...
        if (authDocument.authType == AuthType.RANGE) {
            ...
            if (sigStatus != ProcessingResult.STATUS_VALID_HAS_ANCHOR) {
                error = new Artifact(
                        "IARI Authorization signature does not chain to a known root certificate");
                return error.status;
            }
            ...
            if (!authDocument.iari.substring(Constants.STANDALONE_IARI_PREFIX.length()).equals(
                    keyHash)) {
