...
import com.gsma.iariauth.validator.dsig.KeystoreTrustStore;
...
public final class IARIValidatorMain {
	...
	public static void main(String[] args) {
		...
		opts.addOption(new ArgOption("keystore", "trust roots keystore"));
		opts.addOption(new ArgOption("storepass", "trust roots keystore password"));
		...
		String ksPath = cli.getOptionValue("keystore");
		String ksPasswd = cli.getOptionValue("storepass");
		KeyStore ks = loadKeyStore(ksPath, ksPasswd);
		if(ks == null) {
			System.err.println("Unable to load key store");
			System.exit(1);
		}
		...
		KeystoreTrustStore trustStore = new KeystoreTrustStore(ks);
		try {
			trustStore.load();
		} catch(KeyStoreException kse) {
			System.err.println("Unable to load certificates from trust store");
			System.exit(1);
		}

		PackageProcessor processor = new PackageProcessor(trustStore, packageName, packageSigner);
